...
export default function NewsSection({ TK }: { TK: Tk }) {
  ...
  }, [scope, catFilter, sentFilter, srcFilter, search, selectedDay]);
  ...
  return (
    <div dir={isAr ? 'rtl' : 'ltr'} style={{ fontFamily: 'var(--font-sans)', display: 'flex', flexDirection: 'column', gap: 0, margin: '-32px -32px -40px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 24px', borderBottom: `1px solid ${TK.cardBorder}`, flexWrap: 'wrap', background: TK.body }}>
        ...
        <button onClick={ingest}
          style={{ ...pill(false), background: TK.btnBg, border: `1px solid ${TK.btnBorder}`, color: TK.fg2 }}>
          + Ingest
        </button>
          ...
          {weekDays.map(date => {
            ...
            return (
              <button key={date} onClick={() => handleDayClick(date)}
                style={{
                  background: isSelected ? 'rgba(59,130,246,0.15)' : TK.card,
                  border: `1px solid ${isSelected ? '#3B82F6' : TK.cardBorder}`,
                  borderRadius: 10, padding: '10px 10px', cursor: 'pointer',
                  textAlign: 'left', minHeight: 80,
                }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: TK.fg3, letterSpacing: '0.08em', marginBottom: 4 }}>{dayName}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: isToday ? '#3B82F6' : TK.fg, marginBottom: 8 }}>{dayNum}</div>
                {cal && cal.total > 0 ? (
                  <>
                    {topCats.map(([cat, cnt]) => (
...
// ── Article Row ───────────────────────────────────────────────────────────────
function ArticleRow({ article: a, TK, isAr, selectMode, selected, onSelect }:
  { article: Article; TK: Tk; isAr: boolean; selectMode: boolean; selected: boolean; onSelect: (k: string) => void }) {
  ...
  return (
    <div onClick={() => selectMode && onSelect(a.key)}
      style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <a href={a.link} target="_blank" rel="noopener noreferrer"
          style={{ fontWeight: 600, fontSize: 14, color: TK.fg, textDecoration: 'none', lineHeight: 1.4, display: 'block', marginBottom: 4 }}>
          {title || a.title}
        </a>
        {summary && (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          {a.tickers.map(t => (
            <span key={t} style={{ fontSize: 10, background: 'rgba(255,255,0,0.08)', color: '#FFFF00', borderRadius: 4, padding: '1px 6px', fontWeight: 700 }}>{t}</span>
          ))}
